...
echo -e "║              Self-Hosted Ops Manager in Kubernetes         ║"
echo -e "╚══════════════════════════════════════════════════════════════╝"

# Step 1: Cleaning Environment
log_step "Step 1: Cleaning Environment"
log_info "Removing old deployments..."
kubectl delete namespace ${OPS_MANAGER_NAMESPACE} --ignore-not-found --force --grace-period=0
kubectl delete namespace ${MONGODB_OPERATOR_NAMESPACE} --ignore-not-found --force --grace-period=0
...
# Create kind cluster with port mappings for Ops Manager (8080) and MongoDB (27017)
# Use different host ports to avoid conflicts if local services are running
kind create cluster --name ${K8S_CLUSTER_NAME} --config - <<EOF
kind: Cluster
...
kubectl wait --for=condition=Ready node/${K8S_CLUSTER_NAME}-control-plane --timeout=300s
log_success "Kubernetes cluster created and ready"

# Step 3: Installing MongoDB Kubernetes Operator
...
helm repo update
helm install mongodb-kubernetes mongodb/mongodb-kubernetes --namespace ${MONGODB_OPERATOR_NAMESPACE} --create-namespace --wait
...
log_success "Phase 1 script updated with MongoDB URI configuration fix"